...
def admin_send(text, key=None):
    for admin in ADMINS:
        send(admin, text, key or ADMIN_KB)
...
def receipt(m):
    ...
    try:
        ...
        # ارسال به هر دو ادمین
        for admin in ADMINS:

            try:

                # طبق API همین کتابخانه:
                # photo می‌تواند مسیر فایل باشد
                bot.send_photo(
                    admin,
                    path,
                    text=caption
...
def admin_list(status):
    ...
    for o in orders[:30]:
        ...
        send(
            o.get("admin_chat", next(iter(ADMINS))),
            f"""📦 سفارش #{o["id"]}
