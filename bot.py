...
def discount_used(uid):
    for o in ORDERS.values():
        if (
            str(o.get("chat_id"))==str(uid)
            and o.get("discount_code")==CODE
        ):
            return True
    return False
...
def discount(m,text):
    ...
    if discount_used(uid):
        send(
            uid,
            "⚠️ این کد تخفیف قبلاً برای حساب شما استفاده شده است.",
            kb([
                [("❌ ندارم","no_discount")],
                [("❌ خروج","cancel")]
            ])
...
def receipt(m):
    ...
    try:
        ...
        text=(
            ...
            f"🎁 تخفیف: {money(o['discount'])} تومان\n"
            f"👤 {o['username']}"
        )
