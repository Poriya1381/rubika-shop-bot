...
def handle(m):
    ...
    if text=="📣 کانال":
        prices(uid,CHANNEL,"📣 ","📣 تعرفه کانال")
        return

    if text=="👥 گروه":
        prices(uid,CHANNEL,"👥 ","👥 تعرفه گروه")
        return

    if text=="⭐ فالور":
        prices(uid,FOLLOWERS,"⭐ ","⭐ تعرفه فالور")
        return
    ...
    if p:
        typ,service,price=p
        create_order(m,service,price,typ)
        return
    ...
    if text=="📦 پیگیری":
        ...
        a=sorted(a,key=oid,reverse=True)
